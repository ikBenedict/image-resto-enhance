...
def Logt(canvas):
    ...
    if(len(pik.shape)<3):
        ...
        for m in range(pik.shape[0]):
            for n in range(pik.shape[1]):
...
def outpil(canvas, imaze, z):
    ...
    #saves the cv2 or scipy.misc image and reads it as pil image
        ...
        inpil=PIL.Image.open("cvoutp.jpg")
        canvas.data.image=inpil
    if z==1:
        imageio.imsave('scoutp.jpg', imaze)
        inpil=PIL.Image.open("scoutp.jpg")
        canvas.data.image=inpil
